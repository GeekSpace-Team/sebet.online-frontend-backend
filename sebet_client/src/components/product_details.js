...
const Details =(props)=>{
    const {AddToMany,dil} = useContext(SebedimContext);

    const [num,setNum]=useState(0);
    const SebedeGosh= async()=>{
        AddToMany(props.product,num);
        console.log(num,"ine numm")
    }

    const manat = dil=="tm"?' manat':" манат";
    return (
        <div className="product">
            ...
            <div className="details">
                ...
                <h4>{props.new_price ? props.new_price+manat: props.old_price+manat} 
                    <span id="old-price" >
                        {props.old_price && (props.old_price+manat)}
                    </span>
                </h4>
                <div className="buttons">
                    ...
                    <button onClick={()=>SebedeGosh()} className="product-sebet-button"><img alt="" src={sebet_icon}/> {dil=="tm"?"Sebete goş":"Добавить в корзину"}</button>
                </div>
